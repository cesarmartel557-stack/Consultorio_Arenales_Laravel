          ...
          <div class="booking-card">
            ...
            <div class="row g-4 align-items-start">
              ...
              <div class="col">
                ...
                @if ($doctor->healthInsurances->whereNotNull('logo')->isNotEmpty())
            ...
            @if ($step === 1)
                ...
                <div class="d-flex align-items-center gap-3 flex-wrap">
                  ...
                  <div class="d-flex align-items-center gap-2 week-nav">
                    ...
                    <span class="week-label">
                      @php($first = $this->week->first()['date'])
                      @php($last = $this->week->last()['date'])
                      {{ $first->day }} al {{ $last->day }} de {{ $meses[$last->month - 1] }} {{ $last->year }}
                    </span>
                    <button type="button" wire:click="nextWeek" aria-label="Semana siguiente">
              ...
              <div class="days-scroll">
                @foreach ($this->week as $day)
                  <button
                    ...
                    class="day-pill {{ $day['has_schedule'] ? '' : 'is-off' }} {{ $selectedDate === $day['date']->toDateString() ? 'is-active' : '' }}"
                  >
                    <small>{{ $dias[$day['date']->dayOfWeek] }}</small>
                    <b>{{ $day['date']->day }}</b>
                    <i>{{ $meses[$day['date']->month - 1] }}</i>
              ...
              <div wire:loading.class="opacity-50">
                @php($slots = $this->slots)
                @if ($slots->isEmpty())
                  <div class="no-slots">El profesional no atiende este día. Probá con otro.</div>
                @else
                  @php($manana = $slots->filter(fn ($s) => (int) substr($s['time'], 0, 2) < 13))
                  @php($tarde = $slots->filter(fn ($s) => (int) substr($s['time'], 0, 2) >= 13))

                  @foreach (['Mañana' => $manana, 'Tarde' => $tarde] as $label => $group)
                    ...
                    <div class="slots">
                      @foreach ($group as $slot)
                        <button
                          ...
                          class="slot {{ $selectedTime === $slot['time'] ? 'is-selected' : '' }}"
                        >{{ $slot['time'] }}</button>
                      @endforeach
                    </div>
                  @endforeach
              ...
              <div class="confirm-bar">
                <div class="sel">
                  @if ($selectedDate && $selectedTime)
                    @php($d = Carbon::parse($selectedDate))
                    Turno seleccionado<b>{{ $dias[$d->dayOfWeek] }} {{ $d->day }} de {{ $meses[$d->month - 1] }} · {{ $selectedTime }} hs</b>
                  @else
                    Ningún horario seleccionado<b>—</b>
                  @endif
                </div>
                <button type="button" class="btn btn-brand" wire:click="goToDetails" @disabled(! $selectedDate || ! $selectedTime)>
                  Pedir Turno
                </button>
              </div>
            @endif
            ...
            @if ($step === 2)
              ...
              <div class="slot-summary mb-4">
                <div class="d-flex align-items-center gap-3">
                  ...
                  <div>
                    ...
                    @php($d = Carbon::parse($selectedDate))
                    <b>{{ $dias[$d->dayOfWeek] }} {{ $d->format('d/m') }} · {{ $selectedTime }} hs</b>
                  </div>
                </div>
              ...
              <form wire:submit="submit" novalidate>
                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="field">
                      <input type="email" class="form-control @error('email') is-invalid @enderror" id="mail" placeholder=" " autocomplete="email" maxlength="120" wire:model="email" />
                  ...
                  <div class="col-12">
                    <div class="field">
                      <select class="form-select @error('health_insurance_id') is-invalid @enderror" id="obra" wire:model="health_insurance_id">
